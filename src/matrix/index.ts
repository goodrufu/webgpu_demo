...
async function init_device() {
  ...
  if (!context) { throw new Error('WebGPU初始化失败') }
  ...
  return { device, format, context, canvasInfo: { width: canvas.width, height: canvas.height } }
}
...
async function init_pipeline(device: GPUDevice, format: GPUTextureFormat) {
  ...
  return { pipeline, modelStore: { modelBuffer, modelData }, mvpStore: { mvpGroup, mvpBuffer } }
}
...
async function init() {
  ...
  const { pipeline, modelStore, mvpStore } = await init_pipeline(device, format)

  // 投影矩阵： 相机视野角度、剪裁宽高比、近平面、远平面
  const perspective = mat4.perspective(Math.PI / 3, canvasInfo.width / canvasInfo.height, 0.1, 1000)
