...
async function init_device() {
  ...
  if (!adapter) { throw new Error('该浏览器不支持WebGPU') }

  const device = await adapter.requestDevice()
  if (!device) { throw new Error('该浏览器不支持WebGPU') }
...
async function draw(device: GPUDevice, context: GPUCanvasContext, pipeline: GPURenderPipeline, textureGroup: GPUBindGroup, format: GPUTextureFormat) {
