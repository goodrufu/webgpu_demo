...
async function init_device() {
  ...
  if (!canvas) { throw new Error('没找到canvas节点') }

  if (!navigator.gpu) { throw new Error('该设备不支持WebGPU') }

  const adapter = await navigator.gpu.requestAdapter()
  if (!adapter) { throw new Error('WebGPU初始化失败') }

  const device = await adapter.requestDevice()
  if (!device) { throw new Error('WebGPU初始化失败') }

  const content = canvas.getContext('webgpu')
  if (!content) { throw new Error('WebGPU初始化失败') }
  ...
  return { device, format, content, canvasInfo: { width: canvas.width, height: canvas.height } }
}
...
async function init_pipeline(device: GPUDevice, format: GPUTextureFormat) {
  ...
  const pipeline = await device.createRenderPipelineAsync({
    layout: device.createPipelineLayout({ bindGroupLayouts: [mvpGroupLayout, textureGroupLayout] }),
    vertex: {
...
function write_vertex_buffer(device: GPUDevice) {
  ...
  device.queue.writeBuffer(modelBuffer, 0, vertexDataF32, 0, vertexDataF32.length)

  return { modelBuffer }
}

function draw(content: GPUCanvasContext, device: GPUDevice, pipeline: GPURenderPipeline,
  mvpGroup: GPUBindGroup, modelBuffer: GPUBuffer, textureGroup: GPUBindGroup) {
...
async function init() {
  ...
  function frame() {
    ...
    device.queue.writeBuffer(mvpBuffer, 0, mvpData as Float32Array, 0, mvpData.length)
